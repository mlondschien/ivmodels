...
from anchor_regression.testing import anderson_rubin_test, pulse_test


@pytest.mark.parametrize("discrete", [True, False])
@pytest.mark.parametrize("p", [1, 5])
def test_pulse_test_tsls(discrete, p, simulate_iv):
    X, Y, A = simulate_iv(
        discrete=discrete,
        p=p,
        seed=0,
...
@pytest.mark.parametrize("discrete", [True, False])
@pytest.mark.parametrize("p", [1, 5])
def test_pulse_anchor(test, discrete, p, simulate_iv):
